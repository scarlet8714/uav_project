...
# ============================================================
#
# "opencv"    = 原本 cv2.VideoCapture(0) 方式
# "gstreamer" = GStreamer MJPEG pipeline
#
# 只要改這一個參數即可切換。
CAMERA_BACKEND = "gstreamer"

# OpenCV 模式使用的 camera index，通常是 0 或 1。
CAMERA_SOURCE = 0

# GStreamer 模式使用的 V4L2 裝置。
GSTREAMER_DEVICE = "/dev/video0"

# 【需要你依相機模式填寫】
# GStreamer 擷取解析度與 FPS。
CAMERA_WIDTH = 1920
CAMERA_HEIGHT = 1080
CAMERA_FPS = 30
...
#
# "nvjpegdec"
#   Jetson 上的 NVIDIA JPEG decoder。
#   適合優先測試高解析度 MJPEG。
#
# "jpegdec"
#   GStreamer 軟體 JPEG decoder。
#   可拿來做對照測試。
#
# 若 nvjpegdec plugin 不存在，可先改成 "jpegdec"。
GSTREAMER_JPEG_DECODER = "nvjpegdec"

# appsink 設定：
#
# True：
#   處理速度跟不上時丟掉舊 frame，只保留最新畫面。
#   即時偵測建議 True。
#
# False：
#   不主動丟 frame，可能累積延遲。
...
# YOLO 推論參數
# 你的 TensorRT engine 已經以 imgsz=960 export，這裡保持一致。
YOLO_IMGSZ = 960
YOLO_CONF = 0.4
YOLO_IOU = 0.45
...
def build_gstreamer_pipeline():
    """
    建立 USB UVC MJPEG camera 的 GStreamer pipeline。
    ...
    注意：
        OpenCV 必須有 GStreamer 支援。
        可用 cv2.getBuildInformation() 檢查：
            GStreamer: YES
    """
...
def create_camera():
    """
    根據 CAMERA_BACKEND 建立相機。

    CAMERA_BACKEND = "opencv"
        使用原本 OpenCV VideoCapture。
    ...
    if backend == "opencv":
        ...
        print("[Camera] Backend: GStreamer")

        pipeline = build_gstreamer_pipeline()

        print("[Camera] GStreamer pipeline:")
        print(pipeline)
        ...
        raise ValueError(
            "CAMERA_BACKEND 只能是 'opencv' 或 'gstreamer'，"
            f"目前值為：{CAMERA_BACKEND!r}"
        )

    if not cap.isOpened():
        raise RuntimeError(
            "Camera 開啟失敗。"
            f" backend={CAMERA_BACKEND!r}。"
            "若使用 GStreamer，請確認 OpenCV 顯示 GStreamer: YES，"
            "並確認 GSTREAMER_DEVICE、解析度、FPS、decoder plugin。"
        )
...
# ------------------------------------------------------------
# 以下曝光 / 白平衡控制主要適合 OpenCV / V4L2 模式。
# 使用 GStreamer 時，某些 camera property 不一定能直接用
# camera.set() 控制，必要時可改用 v4l2-ctl。
# ------------------------------------------------------------
...
# Camera 狀態顯示
#
# OpenCV backend 通常可正常回報。
# GStreamer backend 某些 property 可能回傳 0，
# 這不一定代表 pipeline 沒有使用指定解析度/FPS；
# GStreamer 的 caps 已經在 pipeline 中明確指定。
# ------------------------------------------------------------
